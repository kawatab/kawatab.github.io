`(((tag "KDE")
  ...
  ((tag "Mozc" "Fcitx" "Emacs")
   ...
   (id "2015011801")
   (contents
    (p "使用頻度の高い漢字を少しだけ直接入力できるように変更しました。「早速」「今日」「今月」「そんな感じです」「会合」「人間」「出来」「本当」「金言」「後日」「水上」「水分」「気合」などを変換無しで入力できます。")
    (p "シフト無し")
    (table (tr (td "１２３４５６７８９０−＝"))
	   (tr (td "そこしてょつんいのりち"))
	   (tr (td "はか゗とたくう゘゛きれ"))
	   (tr (td "すけになさっる、。゜・")))

    (p "右シフト")
    (table (tr (td "一二三四五火水木金土週円"))
	   (tr (td "ぁひほふめ早速会合出入"))
	   (tr (td "ぃをらあよ言行ゑ気目見"))
	   (tr (td "ぅへせゅゃ今本当何来感")))

    (p "左シフト")
    (table (tr (td "十百千万億六七八九〇〜≒"))
	   (tr (td "年月日時分ぬえみやぇ「　"))
	   (tr (td "上間ゐ前後まおもわゆ」"))
	   (tr (td "下大中小人むろねーぉ／")))))
